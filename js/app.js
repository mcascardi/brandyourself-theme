...
var User = new Class({
	...
	setUIPref: function(property, value) {
		if($.user.loggedIn) {
			...
			$.get('/user/SetUIPref/'+property+'/'+escape(value));
...
var $_GET = {}, urlReferrer, pageMT;

function updateTimeOnPage() {
	$.getJSON('/sites/track/pageUpdate', { 'mt': pageMT, 'url': document.location.href }, function(data) {
...
$(function(){
	...
	//site ping
	urlReferrer = (typeof $_GET['source'] != undefined && $_GET['source'] == 'sig') ? '\'Email Signature\'' : document.referrer;
	pageMT = 0;

	$.getJSON('/sites/track/page', { 'url': document.location.href, 'referrer': urlReferrer, 'user_id': user_id }, function(data) {
		if (data.mt !== false) {
			pageMT = data.mt;
